...
export function CertificationGrid() {
    return (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {certs.map((c) => (
                <div key={c.id} className="p-6 bg-slate-50 border border-slate-200 hover:border-emerald-500 transition-colors group">
                    <c.icon className="text-slate-400 group-hover:text-emerald-500 mb-4 transition-colors" size={24} />
                    ...
                    <div className="text-[10px] font-mono text-emerald-600 bg-emerald-50/50 inline-block px-2 py-1 rounded-sm">
                        {c.code}
                    </div>
                </div>
            ))}
